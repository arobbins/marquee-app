...
var app=angular.module("app",["ui.bootstrap","ui.router","xeditable"]);app.config(function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("home",{url:"/",templateUrl:"../views/marquees/marquees.html",controller:function($scope){$scope.onrelease=!1}}).state("releases",{url:"/{release}",templateUrl:"../views/marquees/marquees.html",controller:function($scope){$scope.onrelease=!0}})}),app.factory("MarqueeData",function($http){return{getMarqueeData:function(callback){$http.get("js/services/marquees.json").success(callback)}}}),app.controller("MarqueesCtrl",function($scope,$stateParams,MarqueeData){$scope.search={m:""},$scope.keydown=function(){MarqueeData.getMarqueeData(function(results){return $scope.m.marquees=results.marquees})},MarqueeData.getMarqueeData(function(results){return $scope.m=results}),$scope.removeMarquee=function(){$scope.marquees.splice(0,1)},$scope.toggleMarquees=function(){$scope.m.marquees.forEach(function(marquee){return marquee.toggled=!marquee.toggled})},$scope.filterMarquees=function(releaseDate){MarqueeData.getMarqueeData(function(results){$scope.m.marquees=results.marquees.filter(function(marquee){return marquee.release===releaseDate})})}}),app.directive("activerelease",function(){return{restrict:"E",template:'<h2 class="h4 col-lg-12">{{ m.marquees[0].release }} Release</h2>',replace:!0}});