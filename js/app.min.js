...
var app=angular.module("app",["ui.bootstrap","ui.router","xeditable"]);app.factory("marquees",function($http){return{getMarquees:function(callback){$http.get("../js/marquees.json").success(callback)}}}),app.factory("releases",function($http){return{getReleases:function(callback){$http.get("../js/releases.json").success(callback)}}}),app.controller("MarqueeCtrl",function($scope,marquees,releases){marquees.getMarquees(function(results){return $scope.marquees=results}),releases.getReleases(function(results){return $scope.releases=results}),$scope.removeMarquee=function(){$scope.marquees.splice(0,1)},$scope.toggleMarquees=function(){$scope.marquees.forEach(function(marquee){return marquee.toggled=!marquee.toggled})},$scope.filterMarquees=function(releaseDate){$scope.marquees.filter(function(marquee){marquee.release===releaseDate})}});