...
var app=angular.module("app",["ui.bootstrap","ui.router","xeditable"]);app.config(function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("releases",{url:"/{release}",templateUrl:"../views/marquees/marquees.html",controller:function($scope,$stateParams,marqueeData){marqueeData.getMarquees(function(results){return marquees=results}),marqueeData.getReleases(function(results){return $scope.releases=results}),$scope.filterMarquees($stateParams.release)}})}),app.factory("marqueeData",function($http){return{getMarquees:function(callback){$http.get("../js/marquees.json").success(callback)},getReleases:function(callback){$http.get("../js/releases.json").success(callback)}}});var marquees;app.controller("MarqueeCtrl",function($scope,marqueeData){marqueeData.getMarquees(function(results){return marquees=results}),marqueeData.getReleases(function(results){return $scope.releases=results}),$scope.removeMarquee=function(){$scope.marquees.splice(0,1)},$scope.toggleMarquees=function(){$scope.marquees.forEach(function(marquee){return marquee.toggled=!marquee.toggled})},$scope.filterMarquees=function(releaseDate){$scope.marquees=marquees.filter(function(marquee){return marquee.release===releaseDate})}});